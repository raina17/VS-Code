...
#     if client.get_open_orders(symbol + 'USDT') == []:
#         print(f'No Open orders available for {symbol}')
#     else:
#         client.cancel_open_orders(symbol + 'USDT')
#         print(f'Canceled Open orders for {symbol}')

#     #creating stop loss
#     stop_loss = client.new_order(symbol=symbol+'USDT', side='SELL', type='STOP_LOSS_LIMIT', price =float(client.ticker_price(symbol + "USDT")["price"])*.94, stopPrice = float(client.ticker_price(symbol + "USDT")["price"])*.95, quantity=current_holdings[symbol], timeInForce = 'GTC')
